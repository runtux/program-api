...
func main() {
	...
	_, err := os.Stat("/etc/ssl/certs/cert.pem")
	if err != nil {
		log.Fatal("You need to provide a certificate")
	}

	_, err = os.Stat("/etc/ssl/certs/key.unencrypted.pem")
	if err != nil {
		log.Fatal("You need to provide a certificate")
	}
	log.Fatal(server.ListenAndServeTLS("/etc/ssl/certs/cert.pem", "/etc/ssl/certs/key.unencrypted.pem"))
}
